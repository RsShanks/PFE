...
def load_gdelt(filepath: str) -> pd.DataFrame:
    ...
    df["EventDescriptionFinal"] = df["EventDescriptionFinal"].fillna(
...
def create_gdelt_weekly_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["negative_intensity"] = (
        df["GoldsteinScale"].clip(upper=0).abs() * df["NumMentions"]
    )
    ...
    weekly = (
        ...
        .agg(
            ...
            verbal_conflict_mentions=(
                "NumMentions",
                lambda x: x[df.loc[x.index, "is_verbal_conflict"]].sum(),
            ),
            material_conflict_mentions=(
                "NumMentions",
                lambda x: x[df.loc[x.index, "is_material_conflict"]].sum(),
            ),
            verbal_coop_mentions=(
                "NumMentions",
                lambda x: x[df.loc[x.index, "is_verbal_coop"]].sum(),
            ),
            material_coop_mentions=(
                "NumMentions",
                lambda x: x[df.loc[x.index, "is_material_coop"]].sum(),
            ),
            major_country_events=("major_actor_event", "sum"),
            major_country_conflict_mentions=(
                "NumMentions",
                lambda x: x[
                    df.loc[x.index, "major_actor_event"]
                    & df.loc[x.index, "is_material_conflict"]
                ].sum(),
            ),
            usa_conflict_mentions=(
                "NumMentions",
                lambda x: x[
                    df.loc[x.index, "usa_event"]
                    & df.loc[x.index, "is_material_conflict"]
                ].sum(),
            ),
            ...
            middle_east_conflict_mentions=(
                "NumMentions",
                lambda x: x[
                    df.loc[x.index, "middle_east_event"]
                    & df.loc[x.index, "is_material_conflict"]
                ].sum(),
            ),
            usa_china_tension=(
                "negative_intensity",
                lambda x: x[df.loc[x.index, "usa_china_relation"]].sum(),
            ),
            usa_russia_tension=(
                "negative_intensity",
                lambda x: x[df.loc[x.index, "usa_russia_relation"]].sum(),
            ),
        )
        .reset_index()
    )
    ...
    weekly["conflict_ratio"] = weekly["conflict_mentions"] / (
        weekly["total_mentions"] + 1
    )
    weekly["material_conflict_ratio"] = weekly["material_conflict_mentions"] / (
        weekly["total_mentions"] + 1
    )
    weekly["major_conflict_ratio"] = weekly["major_country_conflict_mentions"] / (
        weekly["total_mentions"] + 1
    )
...
def add_market_target(df_features: pd.DataFrame, ticker: str) -> pd.DataFrame:
    ...
    # VIX
    vix = yf.download(
        "^VIX", start=START_DATE, end=END_DATE, auto_adjust=False, progress=False
    )
...
def add_temporal_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["conflict_x_drawdown"] = df["material_conflict_ratio_zscore_52w"] * abs(
        df["drawdown_26w"]
    )
...
def train_model(df: pd.DataFrame):
    ...
    results = test[
        ["week", "weekly_return_current", "future_return_4w", "crisis_level"]
    ].copy()
