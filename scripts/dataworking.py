...
def load_and_preprocess_gdelt(filepath):
    ...
    df = df.dropna(
        subset=["EventDescriptionFinal", "GoldsteinScale", "NumMentions", "AvgTone"]
    )
    ...
    features = (
        ...
        .agg(
            ...
            mat_conf_mentions=(
                "NumMentions",
                lambda x: x[
                    df_gdelt.loc[x.index, "QuadClassLabel"] == "Material conflict"
                ].sum(),
            ),
            ...
            verb_conf_mentions=(
                "NumMentions",
                lambda x: x[
                    df_gdelt.loc[x.index, "QuadClassLabel"] == "Verbal conflict"
                ].sum(),
            ),
        )
        .reset_index()
...
def add_market_data(df_features, ticker="^GSPC", start="2006-01-01", end="2022-12-31"):
    ...
    data_weekly = (
        close_prices.resample("W-MON")
        .last()
        .pct_change()
        .to_frame(name="weekly_return")
    )
...
if __name__ == "__main__":
    ...
    # 4. Sauvegarder
    df_final.to_csv(
        "scripts/events/gdelt_weekly_finance_features_with_returns.csv", index=False
    )
