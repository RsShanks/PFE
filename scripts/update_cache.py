...
def update_live_cache():
    print("=== DÉBUT DE LA MISE À JOUR DU CACHE GEOPREDICT ===")
    ...
    for day in last_7_days:
        try:
            ...
            df_day["SQLDATE"] = pd.to_datetime(
                df_day["SQLDATE"].astype(str), format="%Y%m%d"
            )
            df_day["week"] = df_day["SQLDATE"].dt.to_period("W").dt.to_timestamp()

            # Forcer le format numérique sur tes colonnes cibles
            df_day["GoldsteinScale"] = pd.to_numeric(
                df_day["GoldsteinScale"], errors="coerce"
            ).fillna(0)
            df_day["NumMentions"] = pd.to_numeric(
                df_day["NumMentions"], errors="coerce"
            ).fillna(0)
            df_day["AvgTone"] = pd.to_numeric(
                df_day["AvgTone"], errors="coerce"
            ).fillna(0)

            df_day["negative_intensity"] = (
                df_day["GoldsteinScale"].clip(upper=0).abs() * df_day["NumMentions"]
            )
            # Dans GDELT, QuadClass = 3 correspond aux conflits matériels
            df_day["IsMaterialConflict"] = (
                df_day["QuadClass"].astype(str).str.strip() == "3"
            )
            df_day_agg = (
                df_day.groupby("week")
                .agg(
                    ...
                    mat_conf_mentions=(
                        "NumMentions",
                        lambda x: x[df_day.loc[x.index, "IsMaterialConflict"]].sum(),
                    ),
                )
                .reset_index()
            )
            ...
            if day in last_7_days:
                mask = df_day["Actor1CountryCode"].isin(cibles_geo) | df_day[
                    "Actor2CountryCode"
                ].isin(cibles_geo)
            ...
            print(f"Erreur ignorée pour le jour {day} : {e}")
            continue
    ...
    if all_radar_events:
        ...
        df_radar["RiskScore"] = np.where(
            df_radar["GoldsteinScale"] < 0,
            abs(df_radar["GoldsteinScale"]) * df_radar["NumMentions"],
            0,
        )
        ...
        cols_existantes = [
            col for col in colonnes_radar_front if col in df_radar.columns
        ]
