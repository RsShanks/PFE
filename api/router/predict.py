...
router = APIRouter(prefix="/predict", tags=["Prédictions Boursières"])
...
def predict_live_market(ticker: str):
    ...
    try:
        ...
        if not os.path.exists(cache_path):
            raise HTTPException(
                status_code=500,
                detail="Cache GDELT introuvable. Lancez le script de maj.",
            )
...
def predict_market_direction(data: MarketPredictionInput):
    try:
        # 1. Convertir les données reçues en DataFrame pour sklearn
        ...
        return MarketPredictionOutput(
            prediction=pred, tendance=tendance_str, confiance=confiance
        )
...
def simulate_market_reaction(data: SimulationInput):
    try:
        # 1. Création d'une situation normale/neutre pour les variables cachées
        baseline_features = {
            "nb_events": 500,
...
def predict_crisis(ticker: str):
    ...
    try:
        ...
        if cache_path is None:
            raise HTTPException(
                status_code=400, detail="Cache de crise introuvable pour ce ticker."
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erreur de prédiction crise : {str(e)}"
        )
