...
class MarketPredictionInput(BaseModel):
    ...
    # Marché
    weekly_return_current: float = Field(
        ..., description="Rendement boursier de la semaine en cours"
    )
...
class SimulationInput(BaseModel):
    ...
    simulated_tension: float = Field(
        ..., description="Score de tension (ex: de 0 à 2000)"
    )
    simulated_material_conflicts: int = Field(
        ..., description="Mentions de conflits matériels"
    )
...
class CrisisPredictionOutput(BaseModel):
    ...
    probabilites: Dict[str, float] = Field(
        ..., description="Probabilités par classe en pourcentage"
    )
    confiance_prediction: float = Field(
        ..., description="Probabilité associée à la classe prédite"
    )
    derniere_maj_gdelt: str = Field(
        ..., description="Date de mise à jour du cache GDELT"
    )
